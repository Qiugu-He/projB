...
from mcp_server import OscilloscopeControl
    ...
    with gr.Blocks(theme=gr.themes.Base(),css=".gradio_container { background-color: #f0f0f0; }") as chat:
        ...
        with gr.Row():     
            with gr.Column(scale=10):
                chatbot = gr.Chatbot(label="仪器智能体",height=750,avatar_images=("images/user.jpeg","images/agent.png"))
                with gr.Row():
                    # 
            ...
            with gr.Column(scale=5):
                ...
                if devices:
                    selected_osc =gr.Dropdown(choices=devices,label="选择示波器",interactive=True,value=devices[0],scale=2)
                ...
                with gr.Accordion(label="模型设置",open=True):
                    model =gr.Dropdown(choices=['qwen-max','qwen-plus','qwen-turbo','qwen2.5-7b-instruct','qwen2.5-32b-instruct'],label="选择模型",interactive=True,value="qwen-max",scale=2)
                    temperature = gr.Slider(maximum=2,minimum=0,interactive=True,label="温度参数",step=0.01,value=0.0,scale=2)
        ...
        input_message.submit(fn=user,inputs=[input_message,chatbot],outputs=[input_message,chatbot],queue=False).then(
            fn=get_model_response,inputs=[input_message,chatbot,model,temperature,max_tokens,history_round,knowledge_base,similarity_threshold,chunk_cnt,selected_osc],outputs=[chatbot,chunk_text]
            )
